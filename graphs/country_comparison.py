...
def create_layout():
    return html.Div([
        html.H1("Vergleich von Handelsverläufen mit anderen Ländern"),

        # Dropdown für Auswahl von mehreren Ländern
        dcc.Dropdown(
            id='land_dropdown',
            options=[{'label': land, 'value': land} for land in länder_options],
            value=['Islamische Republik Iran'],  # Defaultwert (eine Auswahl)
            multi=True,  # Ermöglicht die Auswahl mehrerer Länder
            clearable=False,
            style={'width': '50%'}
        ),
...
def update_graph(selected_countries):
    # Filtern der Daten für die ausgewählten Länder
    df_filtered = df_grouped[df_grouped['Land'].isin(selected_countries) &
                             (df_grouped['Jahr'] >= 2008) &
                             (df_grouped['Jahr'] <= 2024)]

    fig = go.Figure()

    # Hinzufügen der Daten für jedes Land
    ...
    # Achsen und Layout
    fig.update_layout(
        title='Vergleich der Handelsverläufe',
        xaxis_title='Jahr',
        yaxis_title='Wert in €',
        yaxis=dict(
            tickvals=tickvals,
