...
def update_graph(selected_land, selected_label):
    df_filtered = df[(df['Land'] == selected_land) & (df['Label'] == selected_label)]

    if df_filtered.empty:
        return go.Figure(), f"Keine Daten für {selected_label} in {selected_land} verfügbar."
    ...
    fig.update_layout(
        title=f'Jährlicher Export- und Importverlauf von {selected_label} mit {selected_land}',
        xaxis_title='Jahr',
        yaxis_title='Wert in €',
        xaxis=dict(
            tickmode='array',
    ...
    info_text = (f"Für {selected_label} zwischen Deutschland und {selected_land} beträgt "
